...
const fillSurrounding = (bombR: number, bombC: number, rows: number, columns: number, grid: GridState['gridCells']) => {
    for(let r = -1; r < 2; r+=1) {
        for(let c = -1; c < 2; c +=1 ) {
            const tR = bombR + r;
            const tC = bombC + c;
            if((tR >=0 && tC >=0) && (tR < rows && tC < columns) && (tC !== bombC || tR !==bombR) && grid[tR][tC].number !== -1) {
                const o = { ...grid[tR][tC] }
                o.number += 1;
                grid[tR][tC] = o;
            }
...
const openSurrounding = (rowIndex: number, colIndex: number, grid: Array<Array<Cell>>, rows: number, columns: number) => {
    for(let r = -1; r < 2; r+=1) {
        for(let c = -1; c < 2; c +=1 ) {
            const tR = rowIndex + r;
            const tC = colIndex + c;
...
export const gridSlice = createSlice({
    ...
    reducers: {
        ...
        initializeGrid: (state, action: PayloadAction<{ rows: number, columns: number, totalBombs: number }>) => {
            ...
            if (rows && columns && totalBombs) {
                ...
                const g = Array(rows).fill(0).map(() => Array(columns).fill({ number: 0, isOpen: false, id: '', isFlagged: false }));
        ...
        handleCellOpen: (state, action: PayloadAction<CellIndex>) => {
            // check for bomb !
            let currentGrid = [...state.gridCells];
            ...
            if (clickCell?.number !== 0) {
                ...
                let temp = openCell(rowIndex, colIndex, currentGrid, state.rows, state.columns);
  ...
  export const { setGrid, setGridCell, initializeGrid, handleCellOpen, handleCellDoubleClick, handleCellFlagging, handleCellDeFlagging } = gridSlice.actions  
